...
export class UpdateStatusOrderUseCase {
  ...
  public execute = async ({ orderNumber, status }: UpdateStatusOrderRequestDTO): Promise<"Ok"> => {
    try {
      ...
      if (error instanceof OrderDomainException || error instanceof PublisherException) {
        throw error;
      }
